...
<div class="container">
    <div class="row">
        <div class="col-lg-5">
            <form action="{{ route('service.store.brand') }}" method="POST" enctype="multipart/form-data">
            @csrf
                <div class="card card-body">
                    <div class="mb-1">
                        <strong class="text-dark">Brand Entry</strong>
                    </div>
                    <div class="mb-1">
        ...
        <div class="col-lg-7">
            <div class="card card-body">
                <table id="datatable" class="userList table dt-responsive table-responsive nowrap">
                    ...
                    <tbody>
                        @foreach ($brand as $key => $content)
                        <tr>
                            <td>{{$key+1}}</td>
                            <td>{{$content->name}}</td>
                            <td>
                                <a class="btn btn-info" href="{{route('service.edit.brand',$content->id)}}"><i class="mdi mdi-pencil"></i></a>
                                <button class="btn btn-danger del" name="{{route('service.delete.brand',$content->id)}}"><i class="mdi mdi-delete"></i></button>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>

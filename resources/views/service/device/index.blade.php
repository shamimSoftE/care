...
<div class="container">
    <div class="row">
        <div class="col-lg-5">
            <form action="{{ route('service.store.device') }}" method="POST" enctype="multipart/form-data">
            @csrf
                <div class="card card-body">
                    <div class="mb-1">
                        <strong class="text-dark">Device Entry</strong>
                    </div>
                    <div class="mb-1">
                    ...
                    <div class="mb-1">
                        <label for="" class="form-label">Device Image (85 X 85)</label>
        ...
        <div class="col-lg-7">
            <div class="card card-body">
                <table id="datatable" class="userList table dt-responsive table-responsive nowrap">
                    ...
                    <tbody>
                        @foreach ($device as $key => $content)
                        <tr>
                            <td>{{$key+1}}</td>
                            <td><img width="50" src="{{asset('/uploads/images/service/'.$content->image)}}" alt=""></td>
                            <td>{{$content->name}}</td>
                            <td>
                                <a class="btn btn-info" href="{{route('service.edit.device',$content->id)}}"><i class="mdi mdi-pencil"></i></a>
                                <button class="btn btn-danger del" name="{{route('service.delete.device',$content->id)}}"><i class="mdi mdi-delete"></i></button>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
